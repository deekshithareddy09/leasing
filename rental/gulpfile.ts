...
gulp.task('copy:cert', () => {
	return gulp.src(['server/cert/**'])
		.pipe(gulp.dest('dist/server/cert'));
});
...
gulp.task('build', function (callback: any) {
	runSequence('clean', 'copy:cert', 'build:server', 'build:client', 'clientResources', 'serverResources', 'libs', 'css', callback);
});
